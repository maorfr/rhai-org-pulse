...
const fromForYou = computed(() => nav.params.value.from === 'for-you')

function goBack() {
  if (fromForYou.value) {
    crossNavigate('ai-impact', 'for-you')
...
<template>
  <div class="space-y-6">
    ...
    <button
      ...
      @click="goBack"
    >
      &larr; {{ fromForYou ? 'Back to For You' : fromRfe ? 'Back to RFE Review' : fromFeatureReview ? 'Back to Feature Review' : fromPlan ? 'Back to Plan' : fromHygieneReport ? 'Back to Hygiene Report' : fromFeatureStatus ? 'Back to Feature Status' : 'Back to Execute' }}
    </button>
